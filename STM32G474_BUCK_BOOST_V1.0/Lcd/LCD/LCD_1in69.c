...
{
	UBYTE DataBuffer[2] = {(Data >> 8) & 0xFF, Data & 0xFF};
	
    LCD_1IN69_DC_1;
    LCD_1IN69_CS_0;
//	DEV_SPI_WRITE((Data >> 8) & 0xFF);
//	DEV_SPI_WRITE(Data & 0xFF);
	DEV_SPI_WRiteBuffer(DataBuffer,2);
...
{
	UBYTE DataBuffer[4][4] = {	{Xstart >> 8, Xstart, (Xend-1) >> 8, Xend-1},
								{(Ystart+20) >> 8, Ystart+20, (Yend+20-1) >> 8, Yend+20-1},
								{(Xstart+20) >> 8, Xstart+20, (Xend+20-1) >> 8, Xend+20-1},
								{Ystart >> 8, Ystart, (Yend-1) >> 8, Yend-1 }};
...
//        LCD_1IN69_SendData_8Bit(Xend-1);
		LCD_1IN69_DC_1;
		DEV_SPI_WRiteBuffer(&DataBuffer[0][0], 4);
...
//        LCD_1IN69_SendData_8Bit(Yend+20-1);
		LCD_1IN69_DC_1;
		DEV_SPI_WRiteBuffer(&DataBuffer[1][0],4);
    }
    else {
        // set the X coordinates
        LCD_1IN69_SendCommand(0x2A);
...
//        LCD_1IN69_SendData_8Bit(Xend+20-1);
		LCD_1IN69_DC_1;	
		DEV_SPI_WRiteBuffer(&DataBuffer[2][0],4);
...
//        LCD_1IN69_SendData_8Bit(Yend-1);
		LCD_1IN69_DC_1;
		DEV_SPI_WRiteBuffer(&DataBuffer[3][0],4);
    }
...
{
    /* Static buffer: DMA requires stable memory address (not stack) */
    static UBYTE HeightBuffer[LCD_1IN69_HEIGHT * 2];
    UWORD i, j;

    /* Fill buffer with color (big-endian RGB565) */
    for (j = 0; j < LCD_1IN69_HEIGHT; j++) {
        HeightBuffer[j * 2]     = (Color >> 8) & 0xFF;
        HeightBuffer[j * 2 + 1] = (Color) & 0xFF;
    }
    ...
    for (i = 0; i < LCD_1IN69_WIDTH; i++) {
        /* Wait for previous DMA to finish before reusing buffer */
        while (HAL_DMA_GetState(&hdma_spi1_tx) != HAL_DMA_STATE_READY);
        while (HAL_SPI_GetState(&hspi1)        != HAL_SPI_STATE_READY);
        DEV_SPI_WRiteBuffer(HeightBuffer, LCD_1IN69_HEIGHT * 2);
    }
    /* Wait for last column DMA to complete */
    while (HAL_DMA_GetState(&hdma_spi1_tx) != HAL_DMA_STATE_READY);
    while (HAL_SPI_GetState(&hspi1)        != HAL_SPI_STATE_READY);
}
...
{
	UBYTE HeightBuffer[LCD_1IN69_HEIGHT*2];
    UWORD i,j;
...
//            DEV_SPI_WRITE(*(Image+i*LCD_1IN69_WIDTH+j));
			HeightBuffer[j*2] = (*(Image+i*LCD_1IN69_HEIGHT+j)>>8)&0xff;
			HeightBuffer[j*2+1] = *(Image+i*LCD_1IN69_WIDTH+j);
        }
		DEV_SPI_WRiteBuffer(HeightBuffer,LCD_1IN69_HEIGHT*2);
    }
...
//            DEV_SPI_WRITE(*(Image+Addr+j));
			HeightBuffer[j*2] = (*(Image+Addr+j)>>8)&0xff;
			HeightBuffer[j*2+1] = *(Image+Addr+j);
        }
		DEV_SPI_WRiteBuffer(HeightBuffer,(Xend-1)*2);
    }
}
