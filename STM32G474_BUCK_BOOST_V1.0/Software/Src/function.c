 ...
 * BD    : ?????õô????  (FAN/LCD/KEY/ECDR/RGBLED/TMP/FLASH) */
...

{
    ...
    /* TMP112?????¦Æ?????????HAL_I2C_MemRxCpltCallback?§Õ??? */
...
/**
  ...
  * @note   ??HAL_ADC_ConvCpltCallback()?§Ñ?ADC??????????
...
/**
  * @brief  ???????????
  * @note   HAL_GPIO_EXTI_Callback() ?§Ö???
  *         ???????????:
  *           - KEY1: ???§Ý??????/??
  *           - KEY2: ???????
  *           - KEY3: ???????
  *           - ENCODE1_KEY: ????/????Ú…???
  * @param  GPIO_Pin  ?????§Ø??GPIO????
  * @retval void
  */
...
/* @brief ????TMP112????????????HAL_I2C_MemRxCpltCallback?§Õ???*/
